...
import { Package, AlertTriangle, Plus, Search, Filter, RefreshCw, Trash2, Edit3, X } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-200 gap-4">
        <div>
          <h1 className="text-2xl font-bold font-serif text-herbal-dark">Siddha Medicines Inventory</h1>
          <p className="text-xs text-slate-500">Track stock levels across Choornam, Kudineer, Thailam, and Lehyam formulations.</p>
        </div>
        ...
        <div className="flex items-center space-x-3 w-full sm:w-auto">
          <select
            ...
            className="px-3 py-2 text-xs border border-slate-300 rounded-xl focus:ring-2 focus:ring-siddha-500 bg-white"
          >
          ...
          <button
            ...
            className="flex items-center space-x-2 bg-herbal-dark hover:bg-siddha-900 text-white text-xs font-semibold px-4 py-2.5 rounded-xl shadow transition-colors shrink-0"
          >
                ...
                {inventory.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-slate-50">
                      <td className="p-4 font-bold text-slate-800 text-sm">{item.medicineName}</td>
                      <td className="p-4">
                      ...
                      <td className="p-4 text-right space-x-2">
                        <button
                          ...
                          className="text-slate-600 hover:text-herbal-dark p-1 rounded hover:bg-slate-100"
                        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 border border-amber-900/20">
            <div className="flex justify-between items-center pb-3 border-b border-slate-100">
              <h3 className="text-lg font-bold font-serif text-herbal-dark">
                {editingItem ? 'Update Medicine Stock' : 'Add Siddha Inventory Item'}
              </h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 pt-4 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-siddha-500"
                />
              </div>
              ...
              <button
                type="submit"
                className="w-full bg-herbal-dark text-white font-semibold py-2.5 rounded-lg shadow hover:bg-siddha-900 transition-colors"
              >
